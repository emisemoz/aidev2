        ...
        if results.multi_face_landmarks :
            for face_landmarks in results.multi_face_landmarks : 
                ...
                self.mpDraw.draw_landmarks(image = output_image, 
                                    ...
                                    landmark_drawing_spec = None,
                                    connection_drawing_spec = self.mpDrawStyles.get_default_face_mesh_countours_style())
            return np.ascontiguousarray(output_image[:, :, ::-1], 
                                        dtype = np.uint8), results
    def isOpen(self, image, face_mesh_results, face_part,
            threshold=5) :
        image_height, image_width, _ = image.shape
        output_image = image.copy()
        status = {}
        if face_part == 'MOUTH' : 
            INDEXES = self.mpFaceMesh.MOUTH
        elif face_part == 'LEFT EYE' : 
            INDEXES = self.mpFaceMesh.LEFT_EYE
        elif face_part == 'RIGHT EYE' : 
            INDEXES = self.mpFaceMesh.RIGHT_EYE
        else : 
            return
